datafields = "data/datafields.csv"

nav_image = "data/vti.png"
home_image = "data/home.png"

quarter_1 = "data/q1.csv"
quarter_2 = "data/q2.csv"
quarter_3 = "data/q3.csv"
quarter_4 = "data/q4.csv"

quarter_2_datasales = "data/q2_datesales.csv"
quarter_1_datasales = "data/q1_datesales.csv"
quarter_3_datasales = "data/q3_datesales.csv"
quarter_4_datasales = "data/q4_datesales.csv"

# features = ["DayOfWeek","DayOfWeekName","MonthName","Customers","Promo","StateHoliday","SchoolHoliday","StoreType", \
#     "Assortment","CompetitionDistance","CompetitionOpenSinceMonth","Promo2SinceWeek","PromoInterval"]
# salesDependingFeatures = [6,2,6,437,0,0,0,0,0,4520.0,2.0,14.0,1]
features = ["DayOfWeek","DayOfWeekName","Customers","Promo","StateHoliday","SchoolHoliday","StoreType", \
    "Assortment","CompetitionDistance","CompetitionOpenSinceMonth","Promo2SinceWeek","PromoInterval"]
salesDependingFeatures = [6,2,437,0,0,0,0,0,4520.0,2.0,14.0, 1]

model_load = "model/sales_prediction.sav"
